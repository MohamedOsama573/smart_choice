...
function Wishlist() {
  ...
  useEffect(() => {
    const fetchWishlist = async () => {
      try {
        const response = await axios.get(`${import.meta.env.VITE_BASEURL}api/v1/all-wishlist`, {
          headers: {
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center h-60">
        <p className="text-lg font-semibold animate-pulse">Loading Wishlist...</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
      {wishlist.map((item) => (
        <div key={item.productId} className="border rounded-xl overflow-hidden shadow hover:shadow-lg transition duration-300">
          <img
            src={item.productData.thumbnailImage || item.productData.galleryThumbnails?.[0]}
            alt={item.productData.brand}
            className="w-full h-48 object-cover"
          />
          <div className="p-4 flex flex-col justify-between h-[200px]">
            <div>
              <h3 className="text-lg font-bold text-gray-800 truncate">{item.productData.brand} - {item.modelType}</h3>
              <p className="text-gray-600 text-sm mt-1 truncate">{item.productData.processor}</p>
            </div>
            <div className="mt-4">
